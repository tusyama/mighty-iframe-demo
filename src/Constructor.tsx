...
const WidgetSetup = () => {
    ...
    const [logoSrc, setLogoSrc] = useState('https://cryptologos.cc/logos/toncoin-ton-logo.png')
    ...
    const pagePreviewCode = techStack === 'react'
        ? `<MightyPage\n  partnerId="TON1"\n  targetUrl="${url}"\n  percent="${size}%"\n  theme="${mode}"\n  logoSrc="${logoSrc}"\n />`
        : `<mighty-page\n partnerid="TON1"\n targeturl="${url}"\n percent="${size}%"\n theme="${mode}"\n logoSrc="${logoSrc}"\n >\n</mighty-page>`
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '0', padding: '20px', textAlign: 'start' }}>
            ...
            <div className="form-group">
                ...
                <input type="text" id="urlInput" value={logoSrc} onChange={updateLogoSrc} placeholder="https://test.mighty.study/TON1" />
            </div>
            ...
            <div className="widget-preview">
                <MightyWidget partnerId="TON1" targetUrl={url} percent={`${size}%`} theme={mode} logoSrc={logoSrc}>
            ...
            <pre style={{ paddingTop: '50px', textAlign: "start", position: 'relative' }} className="code-block">{pagePreviewCode}<button onClick={() => copyToClipboard(pagePreviewCode, setCopySuccess)} style={{ position: 'absolute', top: 2, right: 10 }}>{copySuccess ? copySuccess : "Copy"}</button></pre>
